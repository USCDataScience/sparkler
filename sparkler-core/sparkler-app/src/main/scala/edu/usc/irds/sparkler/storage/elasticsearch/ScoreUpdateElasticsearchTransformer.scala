...
object ScoreUpdateElasticsearchTransformer extends (CrawlData => XContentBuilder ) with Serializable with Loggable {

  val fieldMapper: FieldMapper = FieldMapper.initialize()

  override def apply(data: CrawlData): XContentBuilder = {
    ...
    val eUpdate : XContentBuilder = XContentFactory.jsonBuilder()
      ...
      .field(Constants.storage.GENERATE_SCORE, Map("set" -> data.fetchedData.getResource.getGenerateScore()).asJava)
      .endObject()
